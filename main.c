...
int main(int argc, char* argv[])
{
    ...
    while ((ch = getopt(argc, argv, "hp:r:w:")) != -1) {
        switch (ch) {
            ...
            printf("-p <JSON string>   Parse JSON from a string as argument.\n");
            printf("-r <path>          Read JSON from file.\n");
            printf("-w <path>          Write JSON with format to a file. For example, ./C-JSON -r ./input.json -w ./output.json\n");
            printf("                   [Directly type -w <path> will output a null JSON to the file.]\n");
            printf("\n");
            break;
